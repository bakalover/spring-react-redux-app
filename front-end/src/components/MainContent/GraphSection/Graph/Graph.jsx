...
const Graph = (props) => {
  ...
  const clearCurrent = (canvas, canvasCtx) => {
    ...
    // if(document.getElementById('4x').getAttribute('value') === '4n'){
    //   alert("ye");}
    // else{
    //   alert("no");
    // }
  ...
  const drawCurrent = (canvas, canvasCtx) => {
    ...
    for (let i = 0; i < props.xCurrent.length; i++) {
      const elementX = props.xCurrent[i];
      //console.log(elementX);
      const x  = 150 + elementX/4 * 100;
      const y = 150 - props.yCurrent/4 * 100;
      //alert(props.xCurrent.length);
      //alert(props.xCurrent);
      //masDrown.concat(elementX);
      ...
      canvasCtx.fillStyle = hitCheck(elementX, props.yCurrent, props.rCurrent) ? 'green' : 'red';
      canvasCtx.beginPath();
